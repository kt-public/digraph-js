...
import { DiGraphRaw, Traversal, VertexRawSerialized } from '../../src/digraph-raw';
...
type VertexBuilder = {
  raw: (adjacentTo?: string[]) => VertexRawSerialized;
  adjacentTo: (...adjacentTo: string[]) => VertexBuilder;
};

function makeVertex(id: string): VertexBuilder {
  return {
    raw: (adjacentTo = []) => {
      return { id, adjacentTo };
    },
    adjacentTo(...adjacentTo: string[]) {
      return {
        raw: () => {
          return {
            id,
...
describe('Graph traversal', () => {
  ...
  describe('When providing a root vertex', () => {
    test('Should return an empty array or element when the provided vertex does not exists', () => {
      ...
      const vertices: VertexRawSerialized[] = [vertexB, vertexC];
